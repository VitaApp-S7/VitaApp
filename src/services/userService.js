...
export async function getAllUsers(token) {
  var response = await axios.get(`${url  }public/all`, { headers: { Authorization: `Bearer ${token}` }})
...
export async function GetModalVisable(token) {
  ...
  var response = await axios.get(`${url  }modalvisible`, { headers: { Authorization: `Bearer ${token}` }})
  return response.data
}
